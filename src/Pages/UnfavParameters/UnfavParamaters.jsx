...
const UnfavParamaters = () => {
  const [unFavParameters, setUnFavParameters] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchParameters = async (monthId) => {
      setLoading(true);
      try {
        const { status, results } = await getUnFavParameters(monthId);
        if (!isMounted) return;
        if (status) {
        ...
        console.error(err);
        if (isMounted) setUnFavParameters([]);
      } finally {
    ...
    if (cache[activeMonth]) {
      setUnFavParameters(cache[activeMonth]);
      setLoading(false);
    } else {
  ...
  const currentSlice = unFavParameters.slice(startIdx, startIdx + itemsPerPage);
  ...
  const handleEdit = (item) => {
    window.open("/unfav_parameters/edit", "_blank");
